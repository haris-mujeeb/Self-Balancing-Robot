...
constexpr float FILTER_COEFF = 0.9996;
...
void mpu6050_base::calculate(){
  ...
  angle_pitch_gyro += gyro_x * 0.0000611;    // Calculate the traveled pitch angle and add this to the angle_pitch variable
  angle_roll_gyro += gyro_y * 0.0000611;     // Calculate the traveled roll and add this to the angle_roll variable

  //NOTE: 0.000001066 = 0.0000611 * (3.142(PI) / 180degr) The Arduino sin function is in radians
  angle_pitch_gyro += angle_roll_gyro * sin(gyro_z * 0.000001066);    //If the IMU has yawed transfer the roll angle to the pitch angle
  angle_roll_gyro -= angle_pitch_gyro * sin(gyro_z * 0.000001066);    //If the IMU has yawed transfer the pitch angle to the roll angle
  ...
  if (set_gyro_angles) {  // if the IMU has been running
    angle_pitch_comp = angle_pitch_gyro * FILTER_COEFF + angle_pitch_acc * (1 - FILTER_COEFF);  // Correct the drift of the gyro pitch angle with the accelerometer pitch angle
    angle_roll_comp = angle_roll_gyro * FILTER_COEFF + angle_roll_acc * (1 - FILTER_COEFF);     // Correct the drift of the gyro roll angle with the accelerometer roll angle
  } else { // IMU has just started
    angle_pitch_comp = angle_pitch_acc;  // Set the gyro pitch angle equal to the accelerometer pitch angle
    angle_roll_comp = angle_roll_acc;    // Set the gyro roll angle equal to the accelerometer roll angle
    set_gyro_angles = true;         // Set the IMU started flag 
  };

  // To dampen the pitch and roll angles a complementary filter is used
  angle_pitch_output = angle_pitch_output*0.9 + angle_pitch_comp*0.1;  // Take 90% of the output pitch value and add 10% of the raw pitch 
  angle_roll_output = angle_pitch_output*0.9 + angle_roll_comp*0.1;     // Take 90% of the output roll value and add 10% of the raw roll
...
IMUErrorData mpu6050_base::calculate_IMU_error(HardwareSerial &serialPort) {
  ...
  String debugMsg = "IMU Error Data: AccErrorX=" + String(imuError.AccErrorX)
                ...
                + ", GyroErrorY=" + String(imuError.GyroErrorY)
                + ", GyroErrorZ=" + String(imuError.GyroErrorZ);
...
void mpu6050_base::recalibrate() {
  ...
  for (int i = 0; i < calibration_samples; i++) {
    read_mpu_6050_data();
    sum_angle += angle_pitch_gyro;             // Sum of pitch angles
...
void mpu6050_base::testCompFilt(float alpha){
  Serial.print(angle_pitch_gyro); Serial.print('/');
  Serial.print(angle_pitch_acc); Serial.print('/');
  float temp =  angle_pitch_gyro*alpha + (1-alpha)*angle_pitch_acc;
  Serial.println(temp);
}
